      ...
      <div className="bg-white rounded-3xl p-8 w-full max-w-2xl shadow-[0_20px_56px_rgba(140,151,104,0.22)] border border-[rgba(112,128,144,0.18)] max-h-[90vh] flex flex-col">
        ...
        <header className="flex justify-between items-center mb-6">
          <div className="space-y-1">
            <p className="text-[#8c9768] text-[0.65rem] font-medium uppercase tracking-[0.18em] font-serif" style={{ fontFamily: "var(--font-marcellus)" }}>Categoría</p>
            <h2 className="text-3xl font-serif text-[#4a5568]" style={{ fontFamily: "var(--font-marcellus)" }}>
              {categoria.nombre}
            </h2>
          </div>
          <button
        ...
        <div className="space-y-4 overflow-y-auto flex-1 pr-2 custom-scrollbar">
          {categoria.productos.length === 0 ? (
            <div className="text-center py-12 bg-[#f6f4ef]/30 rounded-2xl border border-dashed border-[rgba(112,128,144,0.2)]">
              <p className="font-serif text-[#708090]" style={{ fontFamily: "var(--font-marcellus)" }}>No hay productos en esta categoría.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {categoria.productos.map((p) => (
                ...
                >
                  <div className="space-y-1">
                    <h3 className="text-lg font-serif text-[#4a5568] group-hover:text-[#b76e79] transition-colors" style={{ fontFamily: "var(--font-marcellus)" }}>{p.nombre}</h3>
                    <div className="flex items-center gap-3">
                      <span className="text-[0.65rem] font-serif font-extrabold text-[#708090] uppercase tracking-wide opacity-60" style={{ fontFamily: "var(--font-marcellus)" }}>Stock Actual:</span>
                      <span className="text-sm font-sans font-medium text-[#4a5568]">{p.stock_actual}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-[#708090] opacity-60 uppercase tracking-tighter mb-0.5 font-serif font-extrabold" style={{ fontFamily: "var(--font-marcellus)" }}>Precio</p>
                    <span className="text-xl font-serif text-[#b76e79] font-medium" style={{ fontFamily: "var(--font-marcellus)" }}>${p.precio}</span>
                  </div>
                </div>
              ))}
            </div>
