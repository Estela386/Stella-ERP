...
export default function InventoryReportSection({ productos, ventas }: Props) {
  const { lowStock, noRotation } = useMemo(() => {
    ...
    for (const p of productos) {
      if (p.stock_actual <= (p.stock_min || 0)) {
        lowStockList.push(p);
      }
      
      // If it exists, has stock, but was NOT sold in the period
      if (p.stock_actual > 0 && !soldProductIds.has(p.id)) {
    ...
    return { lowStock: lowStockList, noRotation: noRotationList };
  }, [productos, ventas]);
