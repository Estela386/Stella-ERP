...
export default function ReceiptModal({ isOpen, onClose, order }: ReceiptModalProps) {
    ...
    try {
      ...
      const finishPDF = () => {
        ...
        order.items.forEach(p => {
          ...
          doc.text(`${p.cantidad} x ${p.nombre_producto.substring(0, 18)}`, 8, y);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Recibo de Compra #${order.id}`}>
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: "8px" }}>
            <h4 style={{ margin: 0, fontSize: "1.2rem", color: "#4a5568", fontWeight: "bold" }}>Stella Joyería</h4>
            <p style={{ margin: 0, fontSize: "0.75rem", color: "#708090" }}>Av. Principal 123, Zapopan</p>
          </div>

          <div style={{ borderBottom: "1px dashed #708090", paddingBottom: "12px" }}>
            <p style={{ margin: "4px 0", fontSize: "0.85rem", display: "flex", justifyContent: "space-between", color: "#4a5568" }}>
              <span style={{ fontWeight: "bold" }}>Fecha:</span> <span>{order.fecha}</span>
            </p>
            <p style={{ margin: "4px 0", fontSize: "0.85rem", display: "flex", justifyContent: "space-between", color: "#4a5568" }}>
              <span style={{ fontWeight: "bold" }}>Folio:</span> <span>{order.id}</span>
            </p>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: "10px", padding: "8px 0" }}>
            {order.items.map((item, idx) => (
              <div key={idx} style={{ fontSize: "0.85rem", display: "flex", justifyContent: "space-between", color: "#4a5568" }}>
                <span>{item.cantidad} x {item.nombre_producto}</span>
                <span style={{ fontWeight: "bold" }}>${(item.precio_unitario * item.cantidad).toLocaleString()}</span>
              </div>
            ))}
          </div>

          <div style={{ borderTop: "1px dashed #708090", paddingTop: "12px", display: "flex", flexDirection: "column", gap: "6px" }}>
            <div style={{ fontSize: "0.85rem", display: "flex", justifyContent: "space-between", color: "#708090" }}>
          ...
          <div style={{ textAlign: "center", marginTop: "16px", borderTop: "1px dashed #708090", paddingTop: "16px" }}>
            <p style={{ margin: 0, fontSize: "0.8rem", color: "#708090", fontStyle: "italic" }}>¡Gracias por su compra!</p>
            <p style={{ margin: "4px 0 0 0", fontSize: "0.7rem", color: "#b76e79", fontWeight: "bold", textTransform: "uppercase" }}>Stella Joyería Artesanal</p>
          </div>
        </div>
