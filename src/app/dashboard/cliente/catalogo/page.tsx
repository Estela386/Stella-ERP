      ...
      {filterConfigs.map(({ cat, opts }) => (
        <FilterSection key={cat} title={cat}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {opts.map((opt: string) => {
              const sel = !!activeFilters.find(
                f => f.category === cat && f.value === opt
              );
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: sel
                      ? `1.2px solid ${ROSE}`
                      : "1.2px solid rgba(112,128,144,0.15)",
    ...
    <div style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          border: activeCount
            ? `1.5px solid ${ROSE}`
            : "1.5px solid rgba(112,128,144,0.22)",
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Image
          src={product?.images?.[0]?.url_imagen || "/placeholder.png"}
          alt={product.name}
          fill
          style={{
            objectFit: "cover",
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg,rgba(183,110,121,0.08),transparent)",
            opacity: hovered ? 1 : 0,
...
function CatalogContent() {
  ...
  useEffect(() => {
    ...
    if (catParam && !["personalizada", "nuevos"].includes(catParam)) {
      ...
      if (
        match &&
        !newFilters.some(f => f.category === "Categoría" && f.value === match)
      ) {
    ...
    if (matParam) {
      const match = materiales.find(
        m => m.toLowerCase() === matParam.toLowerCase()
      );
      if (
        match &&
        !newFilters.some(f => f.category === "Material" && f.value === match)
      ) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const { productos: data, error: err } =
        ...
        if (prods.length > 0) {
          ...
          const derivedMats = Array.from(
            new Set(prods.flatMap(p => p.materiales || []).filter(Boolean))
          ) as string[];
  ...
  const filtered = useMemo(() => {
    ...
    const scored = productos.map(p => {
      ...
      activeFilters.forEach(f => {
        ...
        if (f.category === "Color") {
          const textMatches =
            lowerName.includes(val) || lowerDesc.includes(val);
          const optMatches = p.opciones?.some(
            opt =>
  ...
  const filterConfigs: { cat: FilterCategory; opts: string[] }[] = [
    {
      cat: "Material",
      opts: materiales.length
        ? materiales
        : ["Chapa 18k", "Acero Inoxidable", "Hilo"],
    },
          ...
          <div className="flex flex-col md:flex-row gap-8 pt-6 md:pt-10">
            ...
            <div className="flex-1 min-w-0">
                        ...
                        {(["grid", "list"] as const).map(m => (
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: 8,
                              background:
                                viewMode === m ? "#F6F4EF" : "transparent",
                              color:
                                viewMode === m ? ROSE : "rgba(112,128,144,0.3)",
                              cursor: "pointer",
                              border: "none",
                              transition: "all 0.2s",
                            }}
                      ...
                      >
                        {[
                          ...
                          (priceFilter.min > priceRange.min ||
                            ...
                            ? [
                                {
                                  label: `$${priceFilter.min} - $${priceFilter.max}`,
                                  category: "Precio",
                                  value: "price",
                                } as FilterOption,
                              ]
                          ...
                          <button
                            ...
                            onClick={() =>
                              f.category === "Precio"
                                ? setPriceFilter(priceRange)
                                : toggleFilter(f.category, f.value)
                            }
                            style={{
                              display: "flex",
                              alignItems: "center",
                              ...
                              padding: "5px 12px",
                              borderRadius: 16,
                              border: "1.2px solid rgba(183,110,121,0.2)",
                              background: "white",
                              color: ROSE,
                              fontSize: "0.68rem",
                              fontWeight: 700,
              ...
              >
                {!loading && (
                  <p
                    className="text-[9px] uppercase tracking-[0.25em] font-bold text-slate-400"
                ...
                {loading ? (
                  ...
                  <div style={{ textAlign: "center", padding: "80px 0" }}>
                    ...
                    >
                      <Search
                        size={24}
                        style={{ color: "rgba(112,128,144,0.4)" }}
                      />
                    </div>
                    <h3
                      style={{
