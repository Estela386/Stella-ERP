...
const DEFAULT_CONFIG: HeroConfig = {
  ...
  subheadline: "Descubre accesorios únicos, diseñadas con los mejores materiales.",
  featuredProductIds: [],
};
...
function ProductPill({ product }: { product: ProductoBasic }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 12, background: "rgba(246,244,239,0.92)", backdropFilter: "blur(8px)", border: "1px solid rgba(183,110,121,0.22)", boxShadow: "0 2px 12px rgba(140,151,104,0.15)" }}>
      ...
      <div>
        <p style={{ fontFamily: "var(--font-subtitle)", fontSize: "0.82rem", fontWeight: 600, color: "#4a5568", margin: 0, lineHeight: 1.2 }}>{product.nombre}</p>
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.72rem", color: "#b76e79", margin: 0 }}>${product.precio?.toLocaleString()}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ background: "#f6f4ef", minHeight: "clamp(420px, 55vw, 580px)" }}
      >
        {/* Blobs decorativos */}
        <div className="hero-blob pointer-events-none absolute" style={{ top: -80, right: -60, width: 380, height: 380, borderRadius: "60% 40% 55% 45% / 50% 60% 40% 50%", background: "rgba(183,110,121,0.07)", filter: "blur(2px)" }} />
        <div className="hero-blob-2 pointer-events-none absolute" style={{ bottom: -60, left: -40, width: 320, height: 320, borderRadius: "45% 55% 40% 60% / 60% 40% 55% 45%", background: "rgba(140,151,104,0.07)", filter: "blur(2px)" }} />
        ...
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{ background: "linear-gradient(105deg, rgba(246,244,239,0.88) 0%, rgba(246,244,239,0.60) 45%, rgba(246,244,239,0.18) 100%)" }}
          />
        </div>
        ...
        <div
          ...
          style={{ minHeight: "clamp(420px, 55vw, 580px)", padding: "clamp(32px,5vw,72px) clamp(20px,5vw,72px)" }}
        >
          <p
            className="hero-fade-0"
            style={{ fontFamily: "var(--font-subtitle)", fontSize: "0.65rem", fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "#8c9768", marginBottom: 12 }}
          >
            Stella Joyería
          </p>
          ...
          <h1
            ...
            style={{ fontFamily: "var(--font-title)", fontSize: "clamp(2.4rem,5vw,4.8rem)", fontWeight: 400, lineHeight: 1.08, color: "#4a5568", maxWidth: 540, margin: 0 }}
          >
            {config.headline}<br />
          ...
          <p
            ...
            style={{ fontFamily: "var(--font-sans)", fontSize: "clamp(0.88rem,1.4vw,1rem)", color: "#708090", maxWidth: 400, lineHeight: 1.65, marginTop: 16 }}
          >
            {config.subheadline}
          </p>
          ...
          <div className="hero-fade-3" style={{ display: "flex", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
            <button
              ...
              style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", letterSpacing: "0.04em", fontWeight: 500, background: "#b76e79", color: "#f6f4ef", border: "none", borderRadius: 6, padding: "12px 28px", cursor: "pointer", boxShadow: "0 3px 12px rgba(183,110,121,0.22)", transition: "all 0.22s ease" }}
            ...
            <button
              onClick={() => router.push("/dashboard/cliente/catalogo")}
              style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", letterSpacing: "0.04em", fontWeight: 400, background: "transparent", color: "#708090", border: "1.5px solid rgba(112,128,144,0.35)", borderRadius: 6, padding: "12px 24px", cursor: "pointer", transition: "all 0.22s ease" }}
          ...
          {featuredProducts.length > 0 && (
            <div style={{ display: "flex", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
              {featuredProducts.slice(0, 3).map((p, i) => (
                <div key={p.id} className={i % 2 === 0 ? "pill-float" : "pill-float-2"}>
        ...
        <div
          ...
          style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(183,110,121,0.35), rgba(140,151,104,0.25), transparent)" }}
        />
      </section>
      ...
      {isAdmin && editing && (
        <div
          ...
          style={{ background: "rgba(74,85,104,0.45)", backdropFilter: "blur(4px)" }}
          onClick={e => e.target === e.currentTarget && handleCancel()}
        >
          <div
            className="w-full overflow-y-auto"
            style={{ background: "#f6f4ef", borderRadius: "24px 24px 0 0", boxShadow: "0 -20px 56px rgba(140,151,104,0.22)", maxHeight: "88vh", padding: "clamp(20px,3vw,36px)" }}
          >
            {/* Handle */}
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
              <div style={{ width: 44, height: 4, borderRadius: 4, background: "rgba(112,128,144,0.22)" }} />
            </div>

            {/* Título */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              <h2 style={{ fontFamily: "var(--font-title)", fontSize: "1.8rem", fontWeight: 500, color: "#4a5568", margin: 0 }}>
                Editar <em style={{ color: "#b76e79" }}>Hero</em>
              </h2>
              <button
                onClick={handleCancel}
                style={{ width: 36, height: 36, borderRadius: "50%", border: "1px solid rgba(112,128,144,0.25)", background: "white", color: "#708090", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.9rem" }}
              >
                ✕
              </button>
            </div>
            ...
            <div className="grid md:grid-cols-2" style={{ gap: 32 }}>
              ...
              <div>
                <label style={{ display: "block", fontFamily: "var(--font-subtitle)", fontSize: "0.65rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "#708090", marginBottom: 10 }}>
                  Imagen principal
                </label>

                <div
                  style={{ position: "relative", aspectRatio: "16/9", borderRadius: 14, overflow: "hidden", border: "2px dashed rgba(183,110,121,0.35)", background: "white", boxShadow: "0 4px 14px rgba(140,151,104,0.08)", cursor: "pointer" }}
                ...
                >
                  {(imagePreview || draft.imageUrl) ? (
                    <>
                      ...
                      >
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#b76e79" strokeWidth="2">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                      <div style={{ width: 48, height: 48, borderRadius: 12, background: "rgba(183,110,121,0.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#b76e79" strokeWidth="1.5">
                ...
                <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 14 }}>
                  {([
                    ...
                    { label: "Palabra de acento (color rose)", key: "accentWord" },
                    { label: "Subtítulo", key: "subheadline" },
                  ] as { label: string; key: keyof HeroConfig }[]).map(field => (
                    <div key={field.key}>
                      <label style={{ display: "block", fontFamily: "var(--font-subtitle)", fontSize: "0.63rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.16em", color: "#708090", marginBottom: 6 }}>
                        {field.label}
                      </label>
                      <input
                        type="text"
                        ...
                        style={{ width: "100%", padding: "10px 14px", fontFamily: "var(--font-sans)", fontSize: "0.88rem", color: "#4a5568", background: "white", border: "1px solid rgba(112,128,144,0.22)", borderRadius: 8, outline: "none", boxSizing: "border-box", transition: "border-color 0.2s" }}
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <label style={{ fontFamily: "var(--font-subtitle)", fontSize: "0.65rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "#708090" }}>
                ...
                {loadingProducts ? (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 0" }}>
                    <div className="animate-spin" style={{ width: 28, height: 28, border: "2px solid rgba(183,110,121,0.15)", borderTopColor: "#b76e79", borderRadius: "50%" }} />
                  </div>
                ) : (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, maxHeight: 380, overflowY: "auto", paddingRight: 4 }}>
                    {allProducts.map(product => {
                      const selected = draft.featuredProductIds.includes(product.id);
                      const maxReached = draft.featuredProductIds.length >= 3 && !selected;
                      return (
                        <button
                          key={product.id}
                          disabled={maxReached}
                          onClick={() => toggleProduct(product.id)}
                          style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, border: selected ? "1.5px solid #b76e79" : "1px solid rgba(112,128,144,0.18)", background: selected ? "rgba(183,110,121,0.07)" : "white", cursor: maxReached ? "not-allowed" : "pointer", opacity: maxReached ? 0.4 : 1, boxShadow: selected ? "0 2px 10px rgba(183,110,121,0.12)" : "0 1px 4px rgba(140,151,104,0.06)", transition: "all 0.18s ease", textAlign: "left", width: "100%" }}
                        >
                          <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(112,128,144,0.08)", overflow: "hidden", flexShrink: 0 }}>
                            {product.url_imagen && (
                              <Image src={product.url_imagen} alt={product.nombre} width={36} height={36} style={{ objectFit: "cover", width: "100%", height: "100%" }} />
                            )}
                          </div>
                          ...
                          {selected && (
                            <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#b76e79", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 32, paddingTop: 24, borderTop: "1px solid rgba(112,128,144,0.12)" }}>
              <button
                onClick={handleCancel}
                style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", padding: "10px 24px", borderRadius: 6, border: "1.5px solid rgba(112,128,144,0.25)", background: "transparent", color: "#708090", cursor: "pointer", transition: "all 0.18s" }}
              >
                Cancelar
              </button>
              <button
